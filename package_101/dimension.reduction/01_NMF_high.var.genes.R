...
cell.out=list();out=list();mse.out=list();
for(tc in all.tcs){
  ...
  if(F){
    ...
    DimPlot(sce, reduction = "umap")+ggtitle(tc)
  ...
  cell.out[[tc]]=cell_metadata
  out[[tc]]=nmf_model
  mse.out[[tc]]=mse
  cat('tc',tc,'is done\n')
}
#saveRDS(out,'nmf_marrow.heart.rds')
#saveRDS(cell.out,'nmf.cellmeta_marrow.heart.rds')
#saveRDS(mse.out,'nmf.mse_marrow.heart.rds') #20 tc,40min

saveRDS(out,'nmf_115tc.rds')
saveRDS(cell.out,'nmf.cellmeta_115tc.rds')
saveRDS(mse.out,'nmf.mse_115tc.rds') #20 tc,40min。 115 tc, 3hr
