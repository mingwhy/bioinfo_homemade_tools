...
create_plot<-function(name1,name2){
  ...
  if(class(x)==class(y) & class(y)=='numeric'){
    ...
    pval=tmp$p.value
    plot(x,y,pch=16,xlab=name1,cex.lab=2,cex.main=2,ylab='res (pred.age-chronological.age)',
    ...
    boxplot(tmp$y~tmp$x,xlab=name1, outline=FALSE,cex.lab=2,cex.main=2,ylab='res (age-pred.age)',
