...
if(F){
  ...
  adata=anndata.read_h5ad('sce_minCellCounts_PCA50_pseudo.h5ad')
  adata
  ...
  np.save('MMD_dist_pseudo.npy', distances)
  np.save('MMD_pval_pseudo.npy', p_values)
}
###################################################
library(tidyverse);
...
pickle_data_pseudo <- np$load("MMD_dist_pseudo.npy") #$ python python_run_scallop_batch.py
out_pseudo<-lapply(1:dim(pickle_data_pseudo)[1],function(i) pickle_data_pseudo[i,,])

# get tc names
sce.minCellCounts=readRDS('sce_minCellCounts.rds')
tc.names=sort(names(sce.minCellCounts))
names(out)<-tc.names
names(out_pseudo)<-tc.names

df.out=as.data.frame(Reduce(`rbind`,out))
df.out$tc=rep(tc.names,sapply(out,nrow))
colnames(df.out)[1:3]=c('3m-24m','3m-3m','24m-24m')

df.out_pseudo=as.data.frame(Reduce(`rbind`,out_pseudo))
df.out_pseudo$tc=rep(tc.names,sapply(out_pseudo,nrow))
colnames(df.out_pseudo)[1:3]=c('3m-24m','3m-3m','24m-24m')

## plot
plots<-lapply(tc.names,function(tc){
  #tmp=df.out[df.out$tc==tc,]
  tmp=df.out_pseudo[df.out_pseudo$tc==tc,]
...
length(plots)
pdf('MMD_out_pseudo.pdf',useDingbats = T,width = 16,height = 12)
...
cell.lifespan=data.table::fread('~/Documents/aging_cell.turnover/2021_RonMilo_data/mouse_tc_cell.lifespan_20221103.csv');
