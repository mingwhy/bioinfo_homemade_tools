# https://github.com/ljljolinq1010/expression-noise-across-fly-embryogenesis
# change home dir to: expression-noise-across-fly-embryogenesis-master
library(edgeR) 
library(sva) 
library(data.table) 
library(gridExtra) 
...
setwd('./expression-noise-across-fly-embryogenesis-master/')
...
count.E3 #count data (mapping reads count data)
dim(count.all) #17698   288
...
sum(rownames(count.all) %in% rownames(gene.type)) #17698
gene.type=gene.type[rownames(count.all),,drop=F]
head(gene.type)
table(gene.type)
#count.all<-count.all[which(gene.type=="protein_coding"),] 
#there is a bug here, as nrow(count.all)!=nrow(gene.type)
#ncRNA, roX1 are kept in this dataset
sum(is.na(count.all))
#count.all<-na.omit(count.all)
dim(count.all) #17698   288

grep('FBgn0019661',rownames(count.all)) # 1481
grep('FBgn0019661',rownames(gene.type)) #5795
...
kept=sample.quality[sample.id.retained,]
dim(kept) #250 sample retained
kept$sample.id=rownames(kept)
#data.table::fwrite(kept,'250samples_retained.txt')
...
dim(count.retained) #9898 genes in 250 samples
count.retained[[1]] #read count data
...
dim(count.retained.norm) # 9898 gene X 250 samples
count.retained.norm[1:3,1:3] #log2CPM
...
dim(data.input.retained) #9898  250
table(unlist(lapply(strsplit(colnames(data.input.retained),'\\_'),'[',1)))

stage.color<-c("red" ,"orange" , "green","cyan","blue","purple","forestgreen","grey")
#replicates.numb.retained<-c(34, 34, 36, 25, 26, 22 ,30 ,32)
replicates.numb.retained<-c(36, 34, 36, 26, 26, 22 ,33 ,37)
sum(replicates.numb.retained) #250
...
legend("topright",legend=stage.id, 
       col=stage.color,pch=rep(17,times=8),cex=1,bty="n")
...
dim(minor.dataset) #9898   97
minor.dataset<-minor.dataset[c(97,1:96)]
...
dim(major.dataset) # 9898  155
major.dataset<-major.dataset[c(155,1:154)]

dim(major.dataset) # 9898   155, 1col gene id
dim(minor.dataset) # 9898   97
major.dataset[1:3,1:3]
minor.dataset[1:3,1:3]
data.table::fwrite(major.dataset,'155samples_retained.txt')
