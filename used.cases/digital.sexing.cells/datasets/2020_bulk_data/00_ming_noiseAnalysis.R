...
dim(kept) #239 sample retained
kept$sample.id=rownames(kept)
data.table::fwrite(kept,'239samples_retained.txt')
...
dim(count.retained) #8004 genes in 239 samples
...
dim(count.retained.norm) # 8004gene X 239 samples
...
dim(major.dataset) #8004  151, 1col gene id
dim(minor.dataset) #8004   90
major.dataset[1:3,1:3]
minor.dataset[1:3,1:3]
data.table::fwrite(major.dataset,'150samples_retained.txt')
