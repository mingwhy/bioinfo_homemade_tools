...
markers=pick.genes1$query[c(3,7,8)]
markers
...
rownames(sub.df)
rownames(sub.df)=c("msl-2", "lncRNA:roX1", "Sxl")

test_set=as.matrix(sub.df)
rownames(test_set)

y_pred = predict(classifier, newdata = scale(t(test_set)))
table(y_pred) #cluster1 12, cluster2 10.

sex=substr(names(y_pred),0,1)
keep.sample= sex!='U'
#sex[sex=='U']='F';table(sex) #12F, 10M

table(sex[keep.sample],y_pred[keep.sample])
y_pred[keep.sample]
# the 3 M wrongly assigned to F, are M10,M11,M12
# the 2 F wrongly assiend to M, are F14C_r2, F14D
...
table(test_set$y,y_pred) #female wrongly be recognized as males, males always males
...
# Predicting the Test set results
if(F){
scaled.dataset= scale(dataset[-4])
scaled.dataset=cbind(scaled.dataset,dataset$y)
colnames(scaled.dataset)[4]='y'
