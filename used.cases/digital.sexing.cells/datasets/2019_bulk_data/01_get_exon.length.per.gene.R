...
# download GTF file
#$ wget http://ftp.ensembl.org/pub/release-104/gtf/drosophila_melanogaster/
library(GenomicFeatures)
txdb <- makeTxDbFromGFF("Drosophila_melanogaster.BDGP6.32.104.chr.gtf.gz",format="gtf")
txdb
# more info about BDGP6.32 http://uswest.ensembl.org/Drosophila_melanogaster/Info/Index

# then collect the exons per gene id
exons.list.per.gene <- exonsBy(txdb,by="gene")
names(exons.list.per.gene) #FBgn xxx names
...
length(unique(df.exonic.gene.sizes$gene))#23882
...
data.table::fwrite(df.info,"dmel_geneLength_chr.txt")
