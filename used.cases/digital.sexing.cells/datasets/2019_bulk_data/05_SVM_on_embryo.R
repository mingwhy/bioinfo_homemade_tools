...
if(!file.exists('sample.meta_sex.label.txt')){
...
sum(colnames(sub.df)==df.meta$GSM.id)
head(df.meta)

sub.df=sub.df[,df.meta$cluster!='PB']
df.meta=df.meta[df.meta$cluster!='PB',]
sum(colnames(sub.df)==df.meta$GSM.id) #49 samples
df.meta$cluster.id=as.numeric(factor(df.meta$cluster))
table(df.meta$cluster.id) #1=female, 2=male

data = data.frame(t(sub.df), y =df.meta$cluster.id)
head(data)
...
(label.col=ncol(dataset)) #5nd column is the cluster label

# Feature Scaling for train and test separately
training_set[-label.col] = scale(training_set[-label.col])
test_set[-label.col] = scale(test_set[-label.col])
...
y_pred = predict(classifier, newdata = test_set[-label.col])
...
if(T){
  ...
  dim(dataset) #49 x 5
  x=AnnotationDbi::select(org.Dm.eg.db,keys=colnames(dataset)[-label.col],
                          keytype='FLYBASE',columns=c('SYMBOL'))
  x$SYMBOL #"msl-2"       "lncRNA:roX1" "Sxl"  
  colnames(dataset)[1:(label.col-1)]=x$SYMBOL
  dim(dataset)
  scaled.dataset= scale(dataset[-label.col])
  scaled.dataset=cbind(scaled.dataset,dataset$y)
  colnames(scaled.dataset)[label.col]='y'
  #scaled.dataset$y=as.data.frame(scaled.dataset$y)
  classifier = svm(formula = y ~ .,
                   data = scaled.dataset,
                   type = 'C-classification',
                   kernel = 'linear')
  y_pred = predict(classifier, newdata = scaled.dataset)
  table(scaled.dataset[,label.col],y_pred)
}
saveRDS(classifier,'svm_classifier_3genes_all49samples.rds')
...
colnames(test_set)
colnames(test_set)[1]='msl-2'
#colnames(test_set)=c('msl.2','roX1','roX2','Sxl')
#test_set=test_set[,-3] #remove roX2
