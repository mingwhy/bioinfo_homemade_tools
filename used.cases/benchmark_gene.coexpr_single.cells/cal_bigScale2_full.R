...
compute.atac.network = function (expr.data,feature.file,quantile.p=0.998){
  ...
  #filtering the number of genes used in Dp calcualtion
...
fit.model = function(expr.norm,edges,lib.size,plot.pre.clusters=FALSE){
  ...
  while (TRUE)
    ...
    while (TRUE)
      {
      if (cut.level<0 | abs(cut.level-0)<1e05 ) break #ming
      ...
      cut.level=cut.level-0.01; ##ming
      }
    if ( cut.level-0 > 1e05 ) break #ming
    else
      {
   ...
   print('cut level',cut.level,'ncluster',max(mycl),'\n')  #ming
