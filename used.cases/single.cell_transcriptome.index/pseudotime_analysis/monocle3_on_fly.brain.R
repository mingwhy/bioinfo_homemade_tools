...
## log1p normalize data
dat=NormalizeData(dat)
count.mat=dat@assays$RNA@counts
expr.mat=dat@assays$RNA@data; 
dim(expr.mat);sum(rownames(expr.mat)==rownames(gene.meta)) #12094
rownames(expr.mat)=gene.meta$FBID_KEY

## select cell types
head(cell.meta)
x=cell.meta %>% group_by(annotation,Age)  %>% summarise(ncell=n()) %>% filter(ncell>=200)
unique(x$annotation)
...
#for(tc in all.tcs){
  expression_matrix=expr.mat[,cell.meta$annotation==tc &cell.meta$Age!=0 ]
  dim(expression_matrix)
  # gene filter: expr>=M umi in >=N cells
  geneFilter <- apply(expression_matrix,1,function(x){
    sum(x >= 3) >= 10
  })
  expression_matrix <- expression_matrix[geneFilter, ]
  # quantile normalize
  #expression_matrix <- FQnorm(expression_matrix)
  
  gene.names <- rownames(expression_matrix)
  cell.names <- colnames(expression_matrix)
  
  cell_metadata<-cell.meta[cell.meta$annotation==tc&cell.meta$Age!=0,]
  ...
  cds <- cluster_cells(cds,k=30,cluster_method='leiden',random_seed=123)
  #names(cds@clusters$UMAP) #"cluster_result" "partitions"     "clusters"      
  ...
  if(F){
    plot_cells(cds,
               ...
               color_cells_by = "age", # column from cds@colData
    ...
    # check one gene
    grep('Rpl13a|Cfl1',gene.names)
    aging_genes <- c("Rpl13a",'Cfl1')
    plot_cells(cds,
               genes=aging_genes,
  ...
  #cds <- order_cells(cds)
  cds <- order_cells(cds,root_cells=rownames(cds@colData[cds@colData$Age %in% c(0,1),]))
  ...
  df=cds@colData;
  cds@colData$Age=factor(cds@colData$Age,levels=sort(unique(cds@colData$Age)) )
  p1=plot_cells(cds,
             color_cells_by = "Age",
             show_trajectory_graph = FALSE,
             label_cell_groups=FALSE,
