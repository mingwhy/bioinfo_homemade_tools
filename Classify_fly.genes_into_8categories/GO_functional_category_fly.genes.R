...
# plot cell.type network tree based on network node and edge sharing
...
library(GO.db)
# sessionInfo(); #org.Dm.eg.db_3.8.2   AnnotationDbi_1.46.1
...
write.table(x1,quote=F,row.names=F,col.names=F,
            file='./ext_data/classify_genes_into_8categories/GO_non-coding_RNA.genes.txt') 
...
write.table(x,quote=F,col.names=F,row.names=F,
            file='./ext_data/classify_genes_into_8categories/GO_RNA_binding_proteins.txt')
...
dim(x)
write.table(x,quote=F,col.names=F,row.names=F,
            file='./ext_data/classify_genes_into_8categories/GO_synaptic.genes.txt')
