...
head(cellInfo)
dim(cellInfo) #200 cells
dim(exprMat)#862 genes by 200 cells


### Initialize settings
scenicOptions <- initializeScenic(org="mgi", dbDir="cisTarget_databases", nCores=2)
...
dim(exprMat_filtered) #770 genes by 200 cells
...
dim(exprMat_log) #862 x 200

scenicOptions@settings$dbs <- scenicOptions@settings$dbs["10kb"] # Toy run settings
scenicOptions <- runSCENIC_1_coexNetwork2modules(scenicOptions)
scenicOptions <- runSCENIC_2_createRegulons(scenicOptions, coexMethod=c("top5perTarget")) # Toy run settings

BiocManager::install('gridBase',force=T)
